...
import { getPages as getPagesWithSidecar, getPage as getPageWithSidecar } from './pageService';
...
export async function exportPagesAsJSON(): Promise<void> {
  // Use getPagesWithSidecar to ensure origin hashes are enriched from sidecar table
  const pages = await getPagesWithSidecar();
...
export async function exportPagesAsZIP(
  ...
  // CRITICAL: This export is sufficient for independent verification without Umarise
  const readme = `# Umarise Export — Self-Contained Audit Bundle

> **This export is sufficient for independent verification without Umarise.**
...
| \`metadata.json\` | Complete page data (OCR, summaries, keywords) |
| \`images/\` | Original captured artifacts |
...
2. For each entry with \`hash_status: "verified"\`:
   - Locate the image at \`filename\`
   - Calculate SHA-256 of the file bytes
...
\`\`\`powershell
Get-FileHash images\\<page_id>.jpg -Algorithm SHA256
\`\`\`

Compare output to \`origin_hash_sha256\` in manifest.

## Legacy Pages

Pages with \`hash_status: "legacy_no_hash"\` were captured before origin hash
verification. These remain valid origin records but cannot be cryptographically
verified. No retroactive hashing is possible.

## Origin Bytes Definition
...
Generated by Umarise Origin Hash v1.0 (Frozen Specification)
`;
...
export async function exportSelectedPagesAsJSON(pageIds: string[]): Promise<void> {
  // Use getPagesWithSidecar to ensure origin hashes are enriched from sidecar table
  const allPages = await getPagesWithSidecar();
...
export async function getExportStats(): Promise<{ pageCount: number; oldestPage: Date | null; newestPage: Date | null }> {
  // Use getPagesWithSidecar for consistency (though stats don't need hashes)
  const pages = await getPagesWithSidecar();
