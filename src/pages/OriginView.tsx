...
export default function OriginView() {
  ...
  // Load the origin image via backend proxy so the browser doesn't need vault auth.
  useEffect(() => {
    if (!originId) return;
    ...
    async function loadOriginArtifact() {
      setArtifactLoading(true);
      setArtifactError(null);

      try {
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/origin-image-proxy?origin_id=${originId}`,
          {
            method: 'GET',
            headers: {
              apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY,
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch origin image (${response.status})`);
        }
        ...
        console.error('[OriginView] Artifact load failed:', e);
        if (!cancelled) {
          setArtifactUrl((prev) => {
            if (prev) URL.revokeObjectURL(prev);
            return null;
          });
          setArtifactError('Stored in Private Vault (image not accessible in this context)');
        }
      } finally {
          ...
          {artifactUrl ? (
            ...
            <div className="rounded-lg overflow-hidden border border-codex-cream/20 bg-codex-ink/30 p-6">
              <div className="flex flex-col items-center justify-center text-center gap-3">
                ...
                <div>
                  <p className="text-codex-cream font-medium">
                    {artifactLoading ? 'Loading origin image…' : 'Stored in Private Vault'}
                  </p>
                ...
                {!artifactLoading && (
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      // Trigger reload by clearing URL and letting effect re-run on same id
                      setArtifactUrl((prev) => {
                        if (prev) URL.revokeObjectURL(prev);
                        return null;
                      });
                      setArtifactError(null);
                      setArtifactLoading(true);
                      // fire-and-forget: effect will run once; this click is a manual retry UX
                      fetch(
                        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/origin-image-proxy?origin_id=${originId}`,
                        {
                          method: 'GET',
                        ...
                        .catch(() => setArtifactError('Still not accessible.'))
