...
export function UniversalDropZone({ onFile, disabled }: UniversalDropZoneProps) {
  ...
  const processFile = useCallback((file: File) => {
    ...
    onFile({ file, previewUrl, mimeType: file.type || 'application/octet-stream', fileName: file.name, fileSize: file.size });
  }, [onFile]);
  ...
  const handleDragEnter = useCallback((e: React.DragEvent) => { e.preventDefault(); dragCounter.current++; setIsDragging(true); }, []);
  ...
  return (
    <div className="w-full">
      <input id={inputId} type="file" onChange={handleFileChange} className="sr-only" disabled={disabled} tabIndex={-1} />
      <div
        role="button"
        ...
        style={{
          minHeight: '148px',
          border: isDragging ? '1px solid hsl(var(--ritual-gold) / 0.6)' : '1px dashed hsl(var(--ritual-gold) / 0.18)',
          background: isDragging ? 'hsl(var(--ritual-gold) / 0.04)' : 'transparent',
      ...
      >
        <AnimatePresence mode="wait">
          {isDragging ? (
            <motion.div key="dragging" className="absolute inset-0 flex flex-col items-center justify-center gap-2 pointer-events-none"
            ...
            <motion.div key="idle" className="absolute inset-0 flex flex-col items-center justify-center gap-3 pointer-events-none"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="flex items-center gap-2 opacity-30">
                <svg viewBox="0 0 20 24" width={14} height={17}><rect x="1" y="1" width="18" height="22" rx="2" fill="none" stroke="hsl(var(--ritual-cream))" strokeWidth="1"/><text x="3" y="15" fontSize="7" fontFamily="serif" fill="hsl(var(--ritual-cream))">PDF</text></svg>
                <svg viewBox="0 0 20 20" width={14} height={14}><line x1="2" y1="10" x2="2" y2="10" stroke="hsl(var(--ritual-cream))" strokeWidth="2" strokeLinecap="round"/><line x1="6" y1="5" x2="6" y2="15" stroke="hsl(var(--ritual-cream))" strokeWidth="2" strokeLinecap="round"/><line x1="10" y1="3" x2="10" y2="17" stroke="hsl(var(--ritual-cream))" strokeWidth="2" strokeLinecap="round"/><line x1="14" y1="5" x2="14" y2="15" stroke="hsl(var(--ritual-cream))" strokeWidth="2" strokeLinecap="round"/><line x1="18" y1="8" x2="18" y2="12" stroke="hsl(var(--ritual-cream))" strokeWidth="2" strokeLinecap="round"/></svg>
...
export function HashingProgress({ fileName, fileSize, progress }: HashingProgressProps) {
  ...
  return (
    <motion.div className="flex flex-col items-center gap-4 w-full px-6" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
      <motion.div animate={{ scale: [1, 1.12, 1], opacity: [0.5, 1, 0.5] }} transition={{ duration: 2.2, repeat: Infinity, ease: 'easeInOut' }}>
        <svg viewBox="0 0 48 48" width={40} height={40}>
          <polygon points="24,4 42,14 42,34 24,44 6,34 6,14" fill="hsl(var(--ritual-gold))" opacity="0.65" />
