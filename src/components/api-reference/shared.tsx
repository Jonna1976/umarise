...
export function CopyBlock({ code }: { code: string }) {
  ...
  return (
    <div className="relative group">
      <pre className="bg-[hsl(220,10%,10%)] border border-[hsl(var(--landing-cream)/0.06)] rounded-md p-4 text-sm font-mono text-[hsl(var(--landing-cream)/0.9)] overflow-x-auto whitespace-pre">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-xs font-mono transition-colors ${
              tab === i
                ...
                : 'text-[hsl(var(--landing-cream)/0.5)] hover:text-[hsl(var(--landing-cream)/0.8)]'
            }`}
          >
            {label}
          </button>
...
export function Param({ name, type, required, desc }: { name: string; type: string; required?: boolean; desc: string }) {
  return (
    <div className="flex gap-3 py-2 border-b border-[hsl(var(--landing-cream)/0.04)] last:border-0">
      ...
      <span className="text-[hsl(var(--landing-cream)/0.45)] text-xs font-mono shrink-0">{type}</span>
      {required && <span className="text-amber-500/70 text-[10px] font-mono uppercase shrink-0">required</span>}
      <span className="text-[hsl(var(--landing-cream)/0.8)] text-sm">{desc}</span>
    </div>
  );
}
  ...
  return (
    <div className="mb-4 pb-4 border-b border-[hsl(var(--landing-cream)/0.08)]">
      <div className="flex items-center gap-3 mb-2 flex-wrap">
        ...
        <code className="text-[hsl(var(--landing-cream)/0.95)] font-mono text-base">{path}</code>
        {badge}
      </div>
    ...
    <div className="space-y-1">
      {errors.map((e) => (
        <div key={e.error} className="flex gap-3 py-1.5 text-sm">
          <span className="text-[hsl(var(--landing-cream)/0.45)] font-mono shrink-0">{e.code}</span>
          <code className="text-red-400/80 font-mono shrink-0">{e.error}</code>
          <span className="text-[hsl(var(--landing-cream)/0.7)]">{e.desc}</span>
        </div>
      ))}
    </div>
  );
...
export function Note({ children }: { children: React.ReactNode }) {
  return (
    <div className="mt-6 flex gap-3 items-start p-3 rounded border border-[hsl(var(--landing-cream)/0.06)] bg-[hsl(var(--landing-cream)/0.02)]">
      ...
      <p className="text-[hsl(var(--landing-cream)/0.7)] text-sm">{children}</p>
    </div>
  );
}
